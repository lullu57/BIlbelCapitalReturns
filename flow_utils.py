...
def get_flow_column(df: pd.DataFrame, flow_columns: Optional[List[str]] = None) -> str:
    ...
    """
    # Respect configured flow column order to avoid ambiguous inputs.
...
def get_flows_for_period(
    ...
    # Normalize all dates to midnight to ensure consistent comparisons.
    df = trades_df.copy()
    df[date_column] = pd.to_datetime(df[date_column]).dt.normalize()
    start_date = pd.to_datetime(start_date).normalize()
    end_date = pd.to_datetime(end_date).normalize()
    
    # Filter based on inclusive parameter. Timing adjustments (e.g., start-of-day
    # inflows vs end-of-day outflows) are handled by the caller.
    if inclusive == 'right':
...
def get_total_flow_for_period(
    ...
    """
    # Delegate to get_flows_for_period so boundary handling is consistent.
