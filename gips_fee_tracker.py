...
4. Flow Timing:
   - Inflows are treated as start-of-day
   - Outflows are treated as end-of-day
   - Inflows on fee day: INCLUDED in fee calculation
   - Outflows on fee day: INCLUDED in fee calculation (not yet withdrawn)
"""
...
def annual_to_monthly_fee(annual_rate: float) -> float:
    ...
    """
    # Use geometric conversion so 12 months compound back to the annual rate.
...
class GIPSFeeTracker:
    ...
    def _get_current_hurdle_threshold(self) -> float:
        ...
        """
        # Hurdle is additive by design (GIPS-style cumulative hurdle).
    ...
    def _get_performance_fee_proration_factor(
        ...
        Inflows: based on days present after the flow (start-of-day).
        ...
        if is_inflow:
            if flow_date > fee_date:
                return 0.0
            # Start-of-day inflow counts the flow date itself.
            days_present = (fee_date - flow_date).days + 1
        else:
    ...
    def _get_mgmt_fee_adjustment_factor(self, flow_date: date, is_inflow: bool) -> float:
        """
        Calculate the proration factor for management fee adjustments.

        Uses days remaining after the flow date, which corresponds
        ...
        if is_inflow:
            # Start-of-day inflow counts the flow date itself.
            days_remaining = (self.current_quarter_end - flow_date).days + 1
        else:
            # End-of-day outflow removes capital after the flow date.
    ...
    def apply_management_fee(
        ...
        Same-day handling (inflows start-of-day, outflows end-of-day):
        - Inflows on fee day: INCLUDED in fee base
    ...
    def apply_management_fee_adjustment(
        ...
        for flow in self.quarter_flows:
            factor = self._get_mgmt_fee_adjustment_factor(flow.date, flow.is_inflow)
    ...
    def apply_performance_fee(
        ...
        # Increment quarters since HWM; each quarter increases the hurdle
        # even if performance is below the threshold.
        ...
        if fee_date is not None and self.quarter_flows:
            for flow in self.quarter_flows:
                ...
                # Inflows on fee day are included (start-of-day). Outflows on fee day
                # are excluded (end-of-day) since they haven't reduced NAV yet.
                if flow.date > fee_date:
                    continue
                if not flow.is_inflow and flow.date >= fee_date:
                    continue
                if flow.is_inflow:
    ...
    def record_flow(
        ...
        Inflows are treated as start-of-day; outflows are end-of-day.
    ...
    def process_month(
        ...
        mgmt_fee = 0.0
        flow_amount = float(flow_amount or 0.0)
        daily_flow_total = None
        ...
        # Record daily flows for proration (inflows start-of-day, outflows end-of-day)
        if daily_flows:
            daily_flow_total = sum(flow_value for _, flow_value in daily_flows)
            if np.isfinite(daily_flow_total):
                flow_amount = float(daily_flow_total)
            for flow_date, flow_value in daily_flows:
                self.add_flow_for_proration(flow_value, flow_date)

        pre_fee_flow_total = 0.0
        post_fee_flow_total = flow_amount
        fee_date = self.current_quarter_end if self.is_quarter_end(month) else None
        if daily_flows and fee_date is not None:
            pre_fee_flow_total = sum(
                flow_value for flow_date, flow_value in daily_flows if flow_date < fee_date
            )
            # Include inflows on fee date (start-of-day); exclude outflows on fee date.
            pre_fee_flow_total += sum(
                flow_value for flow_date, flow_value in daily_flows
                if flow_date == fee_date and flow_value > 0
            )
            post_fee_flow_total = flow_amount - pre_fee_flow_total
        ...
        # Update gross NAV to pre-flow value; flows are applied after return calculation.
        ...
        # Apply pre-fee flows so fee bases include capital present before fee day.
        if fee_date is not None and pre_fee_flow_total != 0.0:
            self.apply_flow(pre_fee_flow_total, f"{year}-{month:02d} pre_fee")
        ...
        # Apply remaining flows after fees (does not affect net index)
        if post_fee_flow_total != 0:
            self.apply_flow(post_fee_flow_total, f"{year}-{month:02d}")
    ...
    def process_monthly_data(
        ...
        daily_flow_provided = isinstance(daily_flows, pd.DataFrame) and not daily_flows.empty
        flow_proration_warning = None
        if not daily_flow_provided and flow_column in df.columns:
            flow_vals = df[flow_column].apply(parse_value)
            if (flow_vals != 0).any():
                flow_proration_warning = (
                    "Daily flows are missing; fee proration cannot be honored for non-zero flows. "
                    "Provide daily flows for GIPS-compliant fees."
                )
                logging.error(flow_proration_warning)
                if self.require_daily_flows:
                    raise ValueError(flow_proration_warning)

        monthly_results = []

        if daily_flow_provided:
        ...
        return {
            ...
            'flow_proration_warning': flow_proration_warning
        }
