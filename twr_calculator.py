...
def _get_report_currency(config: Optional[Config]) -> str:
    """
    Resolve the reporting currency for this run.

    If report_currency is unset, fall back to base_currency so the rest of the
    pipeline behaves exactly as before (no FX conversion).
    """
    if config is None:
        return 'EUR'
...
def _select_fx_rate_column(
    ...
    """
    Identify the FX column to convert from base -> report currency.

    - direct pair: base/report (e.g., EUR/USD) means multiply base by rate.
    - inverse pair: report/base (e.g., USD/EUR) means invert before applying.
    """
...
def _load_fx_rates(config: Optional[Config]) -> Optional[pd.DataFrame]:
    """
    Load FX rates for base -> report conversion.

    The result is a daily series with columns:
      - Date (normalized)
      - Rate (base -> report)
    """
    if config is None:
    ...
    # FX data is only required when base and report currencies differ.
    ...
    # Allow either a named sheet or the first sheet if unspecified.
    ...
    # Select the correct column for base->report conversion.
    ...
    # Normalize columns to a standard Date/Rate shape.
    ...
    if invert:
        # If the sheet stores report/base, invert to base/report.
...
def _apply_fx_rates(
    ...
    """
    Apply FX rates to NAV and flow data.

    - NAV uses Date; flows use When.
    - merge_asof with direction='backward' uses the last available prior rate
      (e.g., previous business day) for missing dates.
    """
    ...
    # Last-available prior rate for NAV dates.
    ...
    # Last-available prior rate for flows.
    ...
    # Convert all known flow columns (if present) into report currency.
...
def _resolve_nav_adjustment_amount(adj: 'NavAdjustmentConfig', client_name: str) -> float:
    distribute = (adj.distribute or 'equal').lower()
    if distribute in ('weights', 'ratio'):
        weights = {str(k): float(v) for k, v in (adj.weights or {}).items() if v is not None}
        if not weights or client_name not in weights:
            return 0.0
        eligible = [c for c in (adj.clients or weights.keys()) if c in weights]
        total_weight = sum(weights[c] for c in eligible)
        if total_weight <= 0:
            return 0.0
        return adj.amount * (weights[client_name] / total_weight)
    if distribute == 'per_account':
        return adj.amount
    # default: equal
    if adj.clients:
        return adj.amount / max(1, len(adj.clients))
    return adj.amount


def _apply_nav_adjustments(
    nav_df: pd.DataFrame,
    ...
    if config is None or not getattr(config, 'nav_adjustments', None):
        return nav_df
    if nav_df is None or nav_df.empty:
        return nav_df
    if 'Date' not in nav_df.columns or 'Net Asset Value' not in nav_df.columns:
        return nav_df
    ...
    for adj in config.nav_adjustments:
        if adj.brokerage and adj.brokerage != brokerage_name:
            continue
        if adj.clients and client_name not in adj.clients:
            continue

        adj_currency = _normalize_currency_code(adj.currency) or base_currency
        if stage == 'base' and adj_currency != base_currency:
            continue
        if stage == 'report' and adj_currency != report_currency:
            continue

        amount = _resolve_nav_adjustment_amount(adj, client_name)
        if not np.isfinite(amount) or amount == 0.0:
            continue

        start_date = pd.to_datetime(adj.start_date, errors='coerce')
        if pd.isna(start_date):
            logging.warning(f"Invalid nav_adjustment start_date for {brokerage_name}_{client_name}: {adj.start_date}")
            continue
        start_date = start_date.normalize()

        df.loc[df['Date'] >= start_date, 'Net Asset Value'] = (
            df.loc[df['Date'] >= start_date, 'Net Asset Value'] - amount
        )
        logging.info(
            f"Applied NAV adjustment -{amount:,.2f} {adj_currency} to {brokerage_name}_{client_name} "
            f"from {start_date.date()} ({stage})"
        )
...
class ReturnsCalculator:
    ...
    def __init__(self, config_path: str = 'config.yaml', input_path: Optional[str] = None):
        ...
        # Currency configuration is resolved once per run to avoid repeated IO.
        self.report_currency = _get_report_currency(self.config)
        self.currency_symbol = _get_currency_symbol(self.report_currency)
        # FX rates are cached and reused for every account in the run.
...
def calculate_sub_period_returns(
    ...
    # Use NAV dates as the canonical daily grid; flows are mapped into these intervals.
    all_dates = pd.concat([
        nav_df['Date']
    ]).sort_values().unique()
    ...
    for i in range(len(all_dates) - 1):
        ...
        # Split inflows (start-of-day) vs outflows (end-of-day) for the period.
        inflow = 0.0
        outflow = 0.0
        total_flow = 0.0
        if not period_flows.empty:
            try:
                flow_col = flow_utils.get_flow_column(period_flows)
                flow_vals = pd.to_numeric(period_flows[flow_col], errors='coerce').fillna(0.0)
                inflow = float(flow_vals[flow_vals > 0].sum())
                outflow = float((-flow_vals[flow_vals < 0]).sum())
        ...
        if start_nav <= 0:
            if start_nav == 0:
                # Zero NAV with an inflow indicates funding from zero; use inflow as base.
                if end_nav == 0 and inflow == 0 and outflow == 0:
                    sub_period_return = 0.0
                elif inflow > 0:
                    sub_period_return = (end_nav + outflow) / inflow - 1.0
                    logging.info(
                        f"Computed return using inflow as base for zero NAV period {start_date} to {end_date}"
                    )
                else:
                    logging.warning(
                        f"Zero NAV with no inflow for period {start_date} to {end_date}; skipping"
                    )
                    continue
            else:
            ...
            start_base = start_nav + inflow
            if start_base > 0:
                # Inflows at start-of-day, outflows at end-of-day.
                sub_period_return = (end_nav + outflow) / start_base - 1.0
            else:
                logging.warning(
                    f"Non-positive start base for period {start_date} to {end_date}; skipping"
                )
                continue
...
def build_daily_returns_direct(nav_df: pd.DataFrame, trades_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Build daily returns using the reference method:
      r_i = (end_nav + outflow) / (start_nav + inflow) - 1
    where inflows are at start-of-day and outflows are at end-of-day.
    """
    # Build a continuous daily series, carrying NAV forward between snapshots.
    ...
    def compute_ret(row):
        ...
        if 'inflow' in row and 'outflow' in row:
            inflow_v = float(row['inflow']) if pd.notna(row['inflow']) else 0.0
            outflow_v = float(row['outflow']) if pd.notna(row['outflow']) else 0.0
        else:
            inflow_v = max(flow_v, 0.0)
            outflow_v = max(-flow_v, 0.0)
        
        start_base = start_v + inflow_v
        if start_base > 0:
            # Normal case: calculate return based on start-of-day inflows and end-of-day outflows
            return (end_v + outflow_v) / start_base - 1.0
        else:
...
def build_daily_composite(daily_by_account: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for dt in all_dates:
        ...
        flow_total = inflow_total - outflow_total

        # Calculate composite return with start-of-day inflows and end-of-day outflows
        start_base = start_total + inflow_total
        if start_base > 0:
            # Normal case: calculate return based on starting NAV
            daily_return = (end_total + outflow_total) / start_base - 1.0
        else:
            # No starting value and no flow - no return to calculate
            daily_return = 0.0
...
def build_account_daily_from_nav_flows(
    ...
    For each day in the continuous range between min(Date, When) and max(Date, When):
      ...
      - inflow: sum of positive flows occurring on that day (start-of-day)
      - outflow: sum of absolute negative flows occurring on that day (end-of-day)
      - flow: net flow for the day (inflow - outflow)
    """
    ...
    if isinstance(flow_df, pd.DataFrame) and not flow_df.empty:
        ...
        try:
            # Flow column name depends on brokerage; resolve once per DataFrame.
            flow_col = flow_utils.get_flow_column(f, flow_columns)
            flows_by_day = f.groupby('When')[flow_col].sum().to_dict()
            inflow_by_day = f[f[flow_col] > 0].groupby('When')[flow_col].sum().to_dict()
            outflow_by_day = (-f[f[flow_col] < 0].groupby('When')[flow_col].sum()).to_dict()
        except ValueError:
            flows_by_day = {}
            inflow_by_day = {}
            outflow_by_day = {}
    ...
    for dt in pd.date_range(min_date, max_date, freq='D'):
        ...
        inflow = float(inflow_by_day.get(dt, 0.0))
        outflow = float(outflow_by_day.get(dt, 0.0))
