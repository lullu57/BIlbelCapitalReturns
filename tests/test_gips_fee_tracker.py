...
class TestFlowProration:
    ...
    def test_pre_fee_flows_applied_before_perf_fee(self):
        tracker = GIPSFeeTracker(
            initial_nav=100000.0,
            quarterly_mgmt_fee=0.0,
            fiscal_year_start_month=2
        )

        # Q1: Feb (start) through Apr (end)
        tracker.process_month(
            month=2,
            year=2024,
            gross_nav_end=110000.0,
            flow_amount=0.0,
            monthly_return=0.10
        )
        tracker.process_month(
        ...
        daily_flows = [
            (date(2024, 4, 29), 10000.0),  # pre-fee inflow
            (date(2024, 4, 30), 5000.0),   # fee-day inflow (excluded)
            (date(2024, 4, 30), -2000.0),  # fee-day outflow (included)
        ]

        tracker.process_month(
            month=4,
            year=2024,
            gross_nav_end=123000.0,
            flow_amount=13000.0,
            monthly_return=0.0,
            daily_flows=daily_flows
        )

        perf_events = [e for e in tracker.history if e.get('event') == 'performance_fee']
        assert perf_events
    ...
    def test_missing_daily_flows_raises_when_flows_present(self):
        tracker = GIPSFeeTracker(initial_nav=100000.0)
        ...
        with pytest.raises(ValueError, match='Daily flows are missing'):
            tracker.process_monthly_data(
                monthly_df,
                month_column='month',
