...
class TestAggregationContinuity:
    def test_combined_daily_returns_columns(self):
        account_a = {
            'account_name': 'A',
            'daily_returns': pd.DataFrame({
                'date': pd.to_datetime(['2024-01-01', '2024-01-02']),
                'return': [0.01, 0.0],
                'start_nav': [100.0, 101.0],
                'end_nav': [101.0, 101.0],
                'flow': [0.0, 0.0],
            }),
        }
        ...
        results = {}
        twr.aggregate_client_returns([account_a, account_b], results_dict=results)

        assert 'Combined' in results
        combined_daily = results['Combined']['daily_returns']
